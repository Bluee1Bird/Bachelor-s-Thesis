...
Important information for frames are their source and destination, what kind of function the frame has (URB\_function), whether it is a device descriptor packet or an interface descriptor packet, and if available, the HID data it carries.
...
The output relayed from Tshark is analyzed line by line. The beginning of a frame is marked by a substring matching the pattern \verb|Frame\s\d+:|. With this knowledge, every line after can be stored in the content attribute of the Frame and later be analyzed for information. \\ 
