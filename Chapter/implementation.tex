...
The output relayed from tshark is analyzed line by line. The beginning of a frame is marked by a substring matching the pattern \verb|Frame\s\d+:|. Similarly, if the line features source, destination, URB function, bString, HID Data, or Configuration bmAttributes in the line, their values are extracted and stored for that current frame. If the line contains the substrings 

In either case, the line is added to the content attribute of the current frame to allow further extraction in the cases of device descriptors, interface descriptors or device disconnect packets.
